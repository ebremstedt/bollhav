...
def test_total_does_not_bleed_across_models():
    ...
    lines = [line for line in output.splitlines() if "step_a: finished" in line]
    ...
    lines_b = [line for line in output.splitlines() if "step_b: finished" in line]
...
def test_finished_elapsed_uses_ms_for_sub_second():
    ...
    output = finish_captured(execute)
    lines = [line for line in output.splitlines() if "fast: finished" in line]
...
def test_model_without_set_total_shows_count_only():
    ...
    output = finish_captured(execute)
    lines = [line for line in output.splitlines() if "no_total: finished" in line]
