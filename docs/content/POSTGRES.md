...
A view is `view=True` on the model, not a write mode. Runs `CREATE OR REPLACE VIEW`. Set `query=` on the `Model` with the view's SQL definition, and declare the tables it reads from in `upstream`. No dataframe needed.