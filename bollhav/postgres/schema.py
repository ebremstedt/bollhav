...
def ensure_table(conn: psycopg.Connection, model: Model) -> None:
    ...
    table_id = sql.Identifier(model.target.resolved_name)
    logger.debug(
        "Ensuring table: %s.%s",
    ...
    if model.target.partitioned_by is not None:
        index_name = f"{model.target.resolved_name}_{model.target.partitioned_by}_idx"
        conn.execute(
            sql.SQL(
                ...
                table=sql.Identifier(model.target.resolved_name),
    ...
    if unique_columns:
        constraint_name = f"{model.target.resolved_name}_uq"
