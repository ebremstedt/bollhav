...
class PostgresLibrary:
    """Cross-pipeline model registry, backed by one shared table.

    Construct with the caller-owned state connection (opened in
    `main()`, threaded through the lifecycle hooks). Not model-scoped —
    the registry-mutating methods take the model / name they act on."""
    ...
    def register_model(self, model: "Model") -> None:
        ...
        state = PostgresState(model)
        state_schema = state._state_schema() if has_state_table else None
        state_table = state._state_table() if has_state_table else None
    ...
    def lookup_model(self, full_name: str) -> LibraryEntry | None:
    ...
    def is_satisfied(
        ...
        * `state_schema` / `state_table` are set — look for an `applied`
        ...
        row = self.conn.execute(query, [since, until]).fetchone()
