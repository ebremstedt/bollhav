...
(FastAPI; installs `bollhav==3.0.0rc15` from PyPI and runs `seed.py` on
...
│   ├── pyproject.toml    # fastapi, uvicorn, psycopg, bollhav==3.0.0rc15
│   └── Dockerfile
...
`pyproject.toml` pins `bollhav==3.0.0rc15` from PyPI, so a plain
